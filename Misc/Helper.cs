...
public static class Helper
{
    ...
    private static float MostlyUsedCurve(float db, int from, int to) {
        float factor = Utils.GetLerpValue(from, to, db, clamped: true);
    ...
    public static float DamageMultiplierCurve(float db) {
        return MostlyUsedCurve(db, VoiceConfig.Instance.EnvironmentDecibel, 110);
    }

    public static float ExplodeMultiplierCurve(float db) {
        return MostlyUsedCurve(db, VoiceConfig.Instance.ExplodeDecibel, 115);
    }
}